...
// MockDiveSiteRepository implements the full DiveSiteRepository interface
...
func (m *MockDiveSiteRepository) Create(ctx context.Context, siteReq *models.DiveSiteRequest) (*models.DiveSite, error) {
	args := m.Called(ctx, siteReq)
	return args.Get(0).(*models.DiveSite), args.Error(1)
}

func (m *MockDiveSiteRepository) Update(ctx context.Context, id int, siteReq *models.DiveSiteRequest) (*models.DiveSite, error) {
...
func setupDiveSiteHandler() (*DiveSiteHandler, *MockDiveSiteRepository) {
	mockRepo := new(MockDiveSiteRepository)
	handler := NewDiveSiteHandler(mockRepo)
...
func TestDiveSiteHandler_CreateDiveSite(t *testing.T) {
	...
	siteReq := models.DiveSiteRequest{
		...
		Latitude:    40.7128,
		Longitude:   -74.0060,
		Description: strPtr("A great diving spot"),
	}
