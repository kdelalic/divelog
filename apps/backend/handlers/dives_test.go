...
// MockDiveSiteResolver for testing the dive handler
type MockDiveSiteResolver struct {
	mock.Mock
}

func (m *MockDiveSiteResolver) FindOrCreateDiveSite(ctx context.Context, name string, lat, lng float64) (*models.DiveSite, error) {
	args := m.Called(ctx, name, lat, lng)
	return args.Get(0).(*models.DiveSite), args.Error(1)
}

func (m *MockDiveSiteResolver) GetByID(ctx context.Context, id int) (*models.DiveSite, error) {
	args := m.Called(ctx, id)
	return args.Get(0).(*models.DiveSite), args.Error(1)
}

func (m *MockDiveSiteResolver) GetDiveSiteByDiveID(ctx context.Context, diveID int) (*int, error) {
	args := m.Called(ctx, diveID)
	return args.Get(0).(*int), args.Error(1)
}

func setupDiveHandler() (*DiveHandler, *MockDiveRepository, *MockDiveSiteResolver) {
	mockDiveRepo := new(MockDiveRepository)
	mockDiveSiteRepo := new(MockDiveSiteResolver)
	handler := NewDiveHandler(mockDiveRepo, mockDiveSiteRepo)
...
func setupGinContext(method, url string, body interface{}) (*gin.Context, *httptest.ResponseRecorder) {
	...
	c.Request = req
	c.Set("userID", 1) // Simulate UserIDMiddleware
...
func TestDiveHandler_CreateMultipleDives(t *testing.T) {
	...
	mockDiveRepo.On("CreateMultipleDives", mock.Anything, mock.AnythingOfType("[]*models.Dive")).Return(createdDives, []map[string]interface{}{}, nil)
