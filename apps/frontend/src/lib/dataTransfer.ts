...
const backupDiveSchema = z.object({
  ...
  depth: finiteNumber.positive().max(999.99),
	meanDepth: finiteNumber.min(0).max(999.99).optional(),
  ...
  diveType: z.enum(['recreational', 'training', 'technical', 'work', 'research']).optional(),
	diveMode: z.enum(['OC', 'freedive', 'CCR', 'pSCR']).optional(),
	computer: z.object({
		vendor: optionalText(255),
		model: optionalText(255),
		deviceId: optionalText(255),
		serial: optionalText(255),
		firmware: optionalText(255),
	}).optional(),
...
const CSV_COLUMNS: Array<{ header: string; value: (dive: Dive) => unknown }> = [
  ...
  { header: 'max_depth_m', value: (dive) => dive.depth },
	{ header: 'mean_depth_m', value: (dive) => dive.meanDepth },
  { header: 'duration_min', value: (dive) => dive.duration },
  { header: 'buddy', value: (dive) => dive.buddy },
  { header: 'dive_type', value: (dive) => dive.diveType },
	{ header: 'dive_mode', value: (dive) => dive.diveMode },
	{ header: 'surface_interval_min', value: (dive) => dive.surfaceInterval },
	{ header: 'computer_vendor', value: (dive) => dive.computer?.vendor },
	{ header: 'computer_model', value: (dive) => dive.computer?.model },
	{ header: 'computer_device_id', value: (dive) => dive.computer?.deviceId },
